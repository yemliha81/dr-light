...
class Cam_rengi extends CI_Controller {
	...
	public function variant_list()
	{
		...
		$this->load->view('cam_rengi/variant_list_view', $data);
	}
	
	public function add_variant()
	{
		$this->load->view('cam_rengi/add_variant_view');
	}
	...
	public function update_variant($id)
	{
		$data['variant'] = $this->db->select('*')
			->where('id', $id)
			->get('cam_rengi')->row_array();
		
		$this->load->view('cam_rengi/update_variant_view', $data);
	...
	public function update_variant_post()
	{
		...
		$this->db->update('cam_rengi', $upd, array('id' => $post['id']));
	...
	public function delete_variant($id)
	{
		//todo delete script
		if($id >0){
		    $this->db->delete('cam_rengi', array('id' => $id));
		}
		if($this->db->affected_rows() > 0){
			$this->session->set_flashdata('process', 'success');
		}else{
